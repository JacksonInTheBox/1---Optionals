...
/* The string "34e" cannot be converted to an integer, so userAge is set to nil.
 The optional Int? signifies the failed conversion.*/
/*:
...
let userAgeFixed: Int? = Int(userInputAgeFixed)
print(userAgeFixed)

//:  Now use optional binding to unwrap `userAge`. If `userAge` has a value, print it to the console.
if let unwrappedUserAge = userAgeFixed {
    print(unwrappedUserAge)
} else {
    print("userAgeFixed is nil")
}

let unwrappedUserAgeForDemo: Int = userAgeFixed!
print(unwrappedUserAgeForDemo)
